...
type ProtoPackageSyncer struct {
	...
	existingGoMessages map[string]gocode.MessageInfo
...
func NewProtoPackageSyncer(opts *SyncProtoPackageOptions) *ProtoPackageSyncer {
	return &ProtoPackageSyncer{
		opts:               opts,
		existingGoMessages: make(map[string]gocode.MessageInfo),
	}
}
...
func (s *ProtoPackageSyncer) parseExistingTypes() error {
	...
	messages, err := gocode.ExtractMessageInfoFromGoFiles(dir)
	if err != nil {
		return err
	}
